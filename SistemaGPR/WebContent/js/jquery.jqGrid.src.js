...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addMulti = function(rowid,pos,irow){
			//alert("check:: "+v);
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\" value=\""+rowid+"\" onclick='selectCuartoNivel(this);selectAll(this)'/>",
        ...
        setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
		...
		if(this.p.multiselect) {
			//RDELOSREYES - INICIO - ESTA VALIDACION ES PARA QUE EL CHECKBOX DE SELECCIONAR TODO SOLO FUNCIONE PARA EL NIVEL 1.
			try{
				if(p.id.split("_t").length==1) {
					this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox' style='display:block;' onclick='selectCuartoNivel(this);selectAll(this)' />");
				} else {
					this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox' style='display:none;' onclick='selectCuartoNivel(this)' />");
				}
			} catch(exception){
				this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox' style='display:none;' onclick='selectCuartoNivel(this)' />");
			}
			//RDELOSREYES - FIN - 20110816.
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(gi) {
						v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+t.p.id+"_"+rowid+"\" class=\"cbox\" onclick='selectCuartoNivel(this)'/>";
...
function selectAll(element){
	if(nivelSeleccion==1){
		selectAll1Nivel(element);
		return true;
	}else if(nivelSeleccion==2){
		selectAll2Nivel(element);
		return true;
	}else if(nivelSeleccion==5){
		selectAll5Nivel(element);
		return true;
	}
	
	 var tipoEvento = element.checked;		
	 var arrayPalabras = element.id.split("_");
	 var idUno    = arrayPalabras[1];
	 var idDos    = arrayPalabras[2];
	 var idTres   = arrayPalabras[4];
	 var idCuatro = arrayPalabras[6];
	 var sub1 = document.getElementById("cb_"+idUno+"_"+idDos+"_t");
	 var sub0 = document.getElementById("cb_"+idUno);
	 var sub2 = document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t");
	 var sub3 = document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t");
	 
	 if(sub0 != undefined && arrayPalabras.length == 2){// nivel 0
		 for(var j = 1; j<1000; j++){
			sub1 = document.getElementById("jqg_"+idUno+"_"+j);
			if(sub1 != undefined){
				document.getElementById("jqg_"+idUno+"_"+j).click();				
			}
		 }
	 }	 
	
	if (sub1 != undefined && arrayPalabras.length == 3){// nivel 1
		if (!(!tipoEvento && sub1.checked == false)){
			sub1.click();
		}		 
		 
		for(var i = 1; i<document.getElementById(idUno+"_"+idDos+"_t").rows.length; i++){
			var id = document.getElementById(idUno+"_"+idDos+"_t").rows[i-1].id;
			
			if(document.getElementById(idUno+"_"+idDos+"_t").rows[i].id == undefined || 
			   document.getElementById(idUno+"_"+idDos+"_t").rows[i].id == "" ){
					 
				if(!(!tipoEvento && document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+id+"_t").checked == false)){
					document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+id+"_t").click();
				}
					 
				for(var j = 1; j<document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t").rows.length; j++){
	
					var id2 = document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t").rows[j-1].id;
					if(document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t").rows[j].id == undefined || 
					   document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t").rows[j].id == "" ){
							 
						if(!(!tipoEvento && document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t").checked == false)){
							document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t").click();
						}
	
						if (tipoEvento == true){
							llenaCadena("jqg_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2);
						}else {
							vaciaCadena("jqg_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2);
						}
					}
				}
			}
	 ...
	 else if(sub2 != undefined && arrayPalabras.length == 5){// nivel 2
		 if(!(!tipoEvento && sub2.checked == false)){
			 sub2.click();
			 
			 for(var j = 1; j<document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t").rows.length; j++){
	         
			 	 var id2 = document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t").rows[j-1].id;
				 if(document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t").rows[j].id == undefined || 
					document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t").rows[j].id == "" ){
					 
					document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t").click();
					if(tipoEvento == true){
						llenaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2);
					} else {
						vaciaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2);
					}
					
					//TODO debo llenar la variable global 4 nivel
				 }
			 }
		 }
		   
	 }else if(sub3 != undefined && arrayPalabras.length == 7){//nivel 3
		 if(!(!tipoEvento && sub3.checked == false)){
		 	sub3.click();
			   
			if(tipoEvento == true){
				llenaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro);
			} else {
				vaciaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro);
			}
			
			//TODO aqui agrego todos los datos a la variable global
		 }
	 } 

	 if(arrayPalabras.length == 5){// nivel 2 valida 3 estados
		 validaNivel2(idUno,idDos);		 
	 } else if (arrayPalabras.length == 7){//nivel 3
		 validaNivel3(idUno,idDos,idTres);		 
	 } else if (arrayPalabras.length == 9){//nivel 4
		 
		//alert("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro);  //padre del que acciono		 
		//var registros4nivel = mydataSub; //TODO mandar idpadre para consulta AJAX en subgrid
		//var reg4Original = registros4nivel.length;
		 
		 var reg4Seleccion = 0; //valores verdaderos
		 if(arrCuartoNivel.length != undefined && arrCuartoNivel.length >0){

			for(var i =0; i<arrCuartoNivel.length; i++){
					if(arrCuartoNivel[i].idPadre != undefined && arrCuartoNivel[i].idPadre == "jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro){

							if(arrCuartoNivel[i].selectAll != undefined && arrCuartoNivel[i].selectAll == "/"){
								//todos son verdaderos
								
								if(arrCuartoNivel[i].elimina != undefined && arrCuartoNivel[i].elimina.length >2){
									reg4Seleccion = 2;
								} else {
									reg4Seleccion = 1;
								}
								
							} else if(arrCuartoNivel[i].selecciones != undefined && arrCuartoNivel[i].selecciones.length >2){
								//hay al menos un verdadero
								reg4Seleccion = 2;
							} else {								
								reg4Seleccion = 3;
							}
		 ...
		 if(reg4Seleccion == 3){//si todos son falsos
			 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro).indeterminate = false;
			 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro).checked = false;
			 validaNivel3(idUno,idDos,idTres);
		 } else if(reg4Seleccion == 1){//si todos son verdaderos
			 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro).indeterminate = false;
			 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro).checked = true;
			 validaNivel3(idUno,idDos,idTres);
		 } else if(reg4Seleccion == 2){//si hay almenos un verdadero
			 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro).indeterminate = true;
			 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres).indeterminate = true;
			 document.getElementById("jqg_"+idUno+"_"+idDos).indeterminate = true;		 
		 }		 
	 }
}

function llenaCadena(idPadre){
	
	//TODO mandar idpadre para consulta AJAX en subgrid
	//var registros4nivel = mydataSub; 
	//console.info(registros4nivel);
	
	var cadena = new String;
	cadena = '/';
	
	if (arrCuartoNivel!= undefined){
		for(var i=0; i<arrCuartoNivel.length; i++){
			if(arrCuartoNivel[i].idPadre == idPadre){
				arrCuartoNivel[i].selectAll = cadena;
				return;
			}
		}
	}

	var objNuevo = {"idPadre" : idPadre, "selectAll" : cadena, "selecciones":'', "elimina":''};
	arrCuartoNivel.push(objNuevo);
}

function vaciaCadena(idPadre){
	for (var i=0; i<arrCuartoNivel.length; i++){
		if(arrCuartoNivel[i]!= undefined && arrCuartoNivel[i].idPadre == idPadre ){
			
			arrCuartoNivel[i].selectAll = '';
			arrCuartoNivel[i].selecciones = ''; 
			arrCuartoNivel[i].elimina = ''; 
			break;
		}		
	}
}

function validaNivel2(idUno,idDos){
	 var valoresFalsos = 0;
	 var valoresVerdaderos = 0;
	 var tablaN2 = document.getElementById(idUno+"_"+idDos+"_t");
	 for(var i = 1; i<tablaN2.rows.length; i++){
			 if(tablaN2.rows[i].children[0].children[0] != undefined){
				  if(tablaN2.rows[i].children[0].children[0].checked == false && tablaN2.rows[i].children[0].children[0].indeterminate == false){
						  valoresFalsos++;
				  }else {valoresVerdaderos++;}
			 } 
      }
	 var total = valoresFalsos+valoresVerdaderos;
	 if(total == valoresFalsos){//si todos son falsos
		 document.getElementById("jqg_"+idUno+"_"+idDos).indeterminate = false;
		 document.getElementById("jqg_"+idUno+"_"+idDos).checked = false;
	 } else if(total == valoresVerdaderos){//si todos son verdaderos
		 document.getElementById("jqg_"+idUno+"_"+idDos).indeterminate = false;
		 document.getElementById("jqg_"+idUno+"_"+idDos).checked = true;
	 } else if(valoresVerdaderos >0){ //si hay almenos un verdadero
		 document.getElementById("jqg_"+idUno+"_"+idDos).indeterminate = true;
	 }	
}

function validaNivel3(idUno,idDos,idTres){
	 var valoresFalsos = 0;
	 var valoresVerdaderos = 0;
	 var tablaN3 = document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t");
	 for(var i = 1; i<tablaN3.rows.length; i++){
		 if(tablaN3.rows[i].children[0].children[0] != undefined){
			  if(tablaN3.rows[i].children[0].children[0].checked == false && tablaN3.rows[i].children[0].children[0].indeterminate == false){
					  valoresFalsos++;
			  }else {valoresVerdaderos++;}
		 } 
	 }
	 ...
	 var total = valoresFalsos+valoresVerdaderos;
	 if(total == valoresFalsos){//si todos son falsos
		 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres).indeterminate = false;
		 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres).checked = false;
		 validaNivel2(idUno,idDos);
	 } else if(total == valoresVerdaderos){//si todos son verdaderos
		 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres).indeterminate = false;
		 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres).checked = true;
		 validaNivel2(idUno,idDos);
	 } else if(valoresVerdaderos >0){ //si hay almenos un verdadero
		 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres).indeterminate = true;
		 document.getElementById("jqg_"+idUno+"_"+idDos).indeterminate = true;
...
function validaNivel4(idUno,idDos,idTres,idCuatro){
	 
	var valoresFalsos = 0;
	var valoresVerdaderos = 0;
	var tablaN4 = document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t");
	 
	for(var i = 1; i<tablaN4.rows.length; i++){
		 if(tablaN4.rows[i].children[0].children[0] != undefined){
			  if(tablaN4.rows[i].children[0].children[0].checked == false && tablaN4.rows[i].children[0].children[0].indeterminate == false){
					  valoresFalsos++;
			  }else {valoresVerdaderos++;}
		 } 
	}
...
function buscarSelecciones(idTableForm){
	...
	//rastreo por el primer nivel
	while(document.getElementById(strCadNivel1.replace("XX",cuentaNivel1))!=null && 
		  document.getElementById(strCadNivel1.replace("XX",cuentaNivel1))!=undefined){
		
		if(document.getElementById(strCadNivel1.replace("XX",cuentaNivel1)).checked || 
		   document.getElementById(strCadNivel1.replace("XX",cuentaNivel1)).indeterminate==true){
			cadIdsSeleccion+="**1-"+document.getElementById(strCadNivel1.replace("XX",cuentaNivel1)).parentNode.parentNode.children[2].title;
		}
		
		//rastreo por el segundo nivel
		var cuentaNivel2=1;
		var strCadNivel2="jqg_"+idTableForm+"_"+cuentaNivel1+"_t_YY";//cadena con el nombre del check
		
		while(document.getElementById(strCadNivel2.replace("YY",cuentaNivel2))!=null && 
			  document.getElementById(strCadNivel2.replace("YY",cuentaNivel2))!=undefined){
			
			if(document.getElementById(strCadNivel2.replace("YY",cuentaNivel2)).checked || 
			   document.getElementById(strCadNivel2.replace("YY",cuentaNivel2)).indeterminate==true){
				cadIdsSeleccion+="**2-"+document.getElementById(strCadNivel2.replace("YY",cuentaNivel2)).parentNode.parentNode.children[2].title;
			}
			
			//rastreo por el tercer nivel
			var cuentaNivel3=1;
			var strCadNivel3="jqg_"+idTableForm+"_"+cuentaNivel1+"_t_"+cuentaNivel2+"_t_ZZ"; //cadena con el nombre del check
			
			while(document.getElementById(strCadNivel3.replace("ZZ",cuentaNivel3))!=null && 
				  document.getElementById(strCadNivel3.replace("ZZ",cuentaNivel3))!=undefined){
				
				if(document.getElementById(strCadNivel3.replace("ZZ",cuentaNivel3)).checked || 
				   document.getElementById(strCadNivel3.replace("ZZ",cuentaNivel3)).indeterminate==true){
					cadIdsSeleccion+="**3-"+document.getElementById(strCadNivel3.replace("ZZ",cuentaNivel3)).parentNode.parentNode.children[2].title;
				}
				
				//buscamos elementos del 4to nivel
				for(var j=0; j<arrCuartoNivel.length; j++){
					if(arrCuartoNivel[j].idPadre==strCadNivel3.replace("ZZ",cuentaNivel3)){
						//si no tiene la seleccion de todos los elementos
						if(arrCuartoNivel[j].selectAll!='/'){
							cadIdsSeleccion+="**4-"+arrCuartoNivel[j].selecciones;
						}else{
							//en caso de que tenga la seleccion de todos los elementos
							cadIdsSeleccion+="**4-/"+arrCuartoNivel[j].elimina;
						}
						
						if(cadIdsSeleccion=="**4-" || cadIdsSeleccion=="**4-/"){//evitamos que la cadena se llene con basura en el 4to nivel
							cadIdsSeleccion="";
						}
					}
				}
	...
	return cadIdsSeleccion;
}

function initialCheckGrid(arrayObject, idTableForm, indexTDCode, index, nameCheck){
	
	var cad="cb_"+idTableForm;
	var existePadre=0;
	
	var i = 0;
	for(i = 0; i<arrayObject.length; i++){
		if(arrayObject[i].idPadre==cad){
			existePadre=1;
			break;
		}
	}
	
	var arrayCheck = document.getElementsByName(nameCheck);
	...
	if(existePadre == 0){
		...
		for(var z=0; z<arrayCheck.length; z++){
			
			var titleText = filas[z+1].getElementsByTagName("td")[index].title;
			if(titleText == '-1'){
				titleText = filas[z+1].getElementsByTagName("td")[indexTDCode].title;
			}
			
			var isChecked = arrayCheck[z].checked;
			if(existePadre == 0){				
				
				if(isChecked == true){
					var objNuevo = {"idPadre" : cad, "selectAll":'', "selecciones" : "$$"+titleText, "elimina":'' };
					arrayObject.push(objNuevo);
				}
				...
				if(isChecked == true){
					...
					var isAdded = true;
					var arrySplit = arrayObject[i].selecciones.split("$$");
					for(var u=0;u<arrySplit.length;u++){
						if(arrySplit[u] == titleText){
							isAdded = false
							break;
						}
					}
					...
					if(isAdded){
						arrayObject[i].selecciones+="$$"+titleText;
					}
				}
			}
		...
		for(var z=0; z<arrayCheck.length; z++){
			...
			if(isChecked == true){
				...
				if(isAdded){
					...
					var arryElimina = arrayObject[i].elimina.split("$$");
					for(var u=0;u<arryElimina.length;u++){
						if(arryElimina[u] == titleText){
							arrayCheck[z].checked = false;
							isAdded = false;
							break;
						}
					}
...
function selectCheckGrid(elemento, index, arrayObject, idTableForm){
	...
	if(existePadre==1){    //si el padre existe introducimos o borramos el dato
		if(elemento.checked==true){
			
			var isAdded = true;
			var arrySplit = arrayObject[i].selecciones.split("$$");
			for(var u=0;u<arrySplit.length;u++){
				if(arrySplit[u] == elemento.parentNode.parentNode.children[index].title){
					isAdded = false
					break;
				}
			}
			...
			if(isAdded){
				arrayObject[i].selecciones+="$$"+elemento.parentNode.parentNode.children[index].title;
			}			
			
			//frk: en caso de borrar y seleccionar de nuevo un elemento
			var concatCodesDel = '';
			
			var arryElimina = arrayObject[i].elimina.split("$$");
			for(var u=0;u<arryElimina.length;u++){
				if(arryElimina[u] != elemento.parentNode.parentNode.children[index].title){
					if(arryElimina[u] != ''){
						concatCodesDel +="$$"+arryElimina[u];
					}
				}
			}
			
			arrayObject[i].elimina=concatCodesDel;
			...
			for(var u=0;u<arrySplit.length;u++){
				if(arrySplit[u] != elemento.parentNode.parentNode.children[index].title){
					if(arrySplit[u] != ''){
						concatCodes +="$$"+arrySplit[u];
					}
				}
			}
			
			arrayObject[i].selecciones=concatCodes;			
			
			//guardando el elemento eliminado
			arrayObject[i].elimina+='$$'+elemento.parentNode.parentNode.children[index].title;
		}
	}else{
		var arrayCheck = document.getElementsByName(elemento.name);
		var tabla = document.getElementById(idTableForm);
	  	var tablaBody = tabla.getElementsByTagName("tbody")[0];
	  	var filas = tablaBody.getElementsByTagName("tr");
		...
		for(var z=0; z<arrayCheck.length; z++){
			var isChecked = arrayCheck[z].checked;
			if(isChecked == true){
				var objNuevo = {"idPadre" : cad, "selectAll":'', "selecciones" : "$$"+filas[z+1].getElementsByTagName("td")[index].title, "elimina":'' };
				arrayObject.push(objNuevo);
			}
		}
	}
...
function recordCheckGrid(gridId, arrayObject, idSelect, nameCheck){
	var subGrid = "cb_" + gridId;	
	for ( var i = 0; i < arrayObject.length; i++) {
		if (arrayObject[i] != undefined && arrayObject[i].idPadre != undefined) {
			if (arrayObject[i].idPadre == subGrid) {
				if (arrayObject[i].selectAll == "/") {
					var arrayHijos = jQuery("#" + gridId).getDataIDs();
					var elimindados = new Array();
					if (arrayObject[i].elimina.length > 2) {
						elimindados = arrayObject[i].elimina.split("$$");
					}
					for ( var j = 0; j < arrayHijos.length; j++) {
						var bandera = false;
						for ( var k = 0; k < elimindados.length; k++) {
							if (jQuery("#" + gridId).getRowData(
									arrayHijos[j])[idSelect] == elimindados[k]) {// cambiar por idSelect
								bandera = true;
							}
						}
						if (bandera == false) {
							jQuery("#" + gridId).setSelection(arrayHijos[j], true);
							$("#jqg_"+$.jgrid.jqID(t.p.id)+"_"+$.jgrid.jqID(sr)).attr("checked",false);
						}

					}
				} else if (arrayObject[i].selecciones.length > 2) {
					
					
					var select = new Array();
					select = arrayObject[i].selecciones.split("$$");
					...
					for ( var i = 0; i < arrayHijos.length; i++) {
						...
						for ( var j = 0; j < select.length; j++) {
							if (jQuery("#" + gridId).getRowData(arrayHijos[i])[idSelect] == select[j]) {// cambiar por idSelect
								bandera = true;
							}
						}
						if (bandera == true) {
...
function searchCheckGrid(arrayObject){
	var cadIdsSeleccion = ""; //frk: es el id de los elementos seleccionados
	
	for(var i = 0; i<arrayObject.length; i++){
		cadIdsSeleccion+=arrayObject[i].selecciones;
	}
	
	return cadIdsSeleccion;
}

function selectCuartoNivel(elemento){
	var cad = elemento.id;
	var arrCad2=cad.split("_t_");       //se convierte en arreglo
	var arrAll=cad.split("_");
	
	if(arrCad2.length!=nivelSeleccion){ //valida si estamos en el nivel a paginar
		return false;
	}
	
	if(nivelSeleccion == 1 && arrAll.length == 2){
		return false;
	}
	
	if(nivelSeleccion==4){
		arrCad2.splice(3,1);          //borramos el ultimo elemento para obtener el id padre	
	}else if(nivelSeleccion==2){
		arrCad2.splice(1,2);          //borramos el ultimo elemento para obtener el id padre
	}else if(nivelSeleccion==5){
		arrCad2.splice(4,1);          //borramos el ultimo elemento para obtener el id padre
	}
	
	if(nivelSeleccion!=1){
		cad=arrCad2.join("_t_");      //unimos de nuevo la cadena "ID_PADRE"
	}else{
		cad="cb_"+elemento.parentNode.parentNode.parentNode.parentNode.id;
	}
	
	//buscamos si ya existe el idPadre en el arreglo
	var existePadre=0;
	var i=0;
	for(i=0; i<arrCuartoNivel.length; i++){
		if(arrCuartoNivel[i].idPadre==cad){
			existePadre=1;
			break;
		}
	}
	...
	if(existePadre==1){    //si el padre existe introducimos o borramos el dato
		if(elemento.checked==true){
			...
			if(arrCuartoNivel[i].selecciones.indexOf("$$"+elemento.parentNode.parentNode.children[1].title) == -1){
				arrCuartoNivel[i].selecciones+="$$"+elemento.parentNode.parentNode.children[1].title;
			}			
			
			if(arrCuartoNivel[i].selectAll=='/'){
				var eliminaTem=arrCuartoNivel[i].elimina; //en caso de borrar y seleccionar de nuevo un elemento y tenga la bandera de selecciono todo
				arrCuartoNivel[i].elimina=eliminaTem.replace('$$'+elemento.parentNode.parentNode.children[1].title,'');
			}
		}else{
			arrCuartoNivel[i].selecciones=arrCuartoNivel[i].selecciones.replace("$$"+elemento.parentNode.parentNode.children[1].title,"");
			if(arrCuartoNivel[i].selectAll=='/'){//indicamos cuales fueron los elementos que ya no selecciono
				arrCuartoNivel[i].elimina+='$$'+elemento.parentNode.parentNode.children[1].title;
			}
		}
	}else{                 //si el padre no existe introducimos al padre y al hijo
		if(elemento.checked==true){
			var objNuevo = {"idPadre" : cad, "selectAll":'', "selecciones" : "$$"+elemento.parentNode.parentNode.children[1].title, "elimina":'' };
			arrCuartoNivel.push(objNuevo);				
		}
	}
}
...
function seleccionNivelSuperior(idActual){
	if(document.getElementById(idActual).checked){
		var arrCad2=idActual.split("_t_");      //se convierte en arreglo
		for(var i=arrCad2.length; i>1; i--){
			arrCad2.splice(arrCad2.length-1,1); //borramos el ultimo elemento para obtener el id padre
			idActual=arrCad2.join("_t_");       //unimos de nuevo la cadena "ID_PADRE"
			document.getElementById(idActual).checked=true;
		}	
	}
}

/*************************** REGLAS PARA EL PAGINADO EN EL QUINTO NIVEL ****************************/
function selectAll5Nivel(element){
	...
	var idTres   = arrayPalabras[4];
	var idCuatro = arrayPalabras[6];
	var idCinco  = arrayPalabras[8];
	var sub0 = document.getElementById("cb_"+idUno);
	var sub1 = document.getElementById("cb_"+idUno+"_"+idDos+"_t");	
	var sub2 = document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t");
	var sub3 = document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t");
	var sub4 = document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco+"_t");
	...
	if (sub1 != undefined && arrayPalabras.length == 3){// nivel 1
		...
		for(var i = 1; i<document.getElementById(idUno+"_"+idDos+"_t").rows.length; i++){
				...
				for(var j = 1; j<document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t").rows.length; j++){
					...
					if(document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t").rows[j].id == undefined || 
					   document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t").rows[j].id == "" ){
						...
						for(var z = 1; z<document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t").rows.length; z++){
							
							var id3 = document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t").rows[z-1].id;
							if(document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t").rows[z].id == undefined || 
							   document.getElementById(idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t").rows[z].id == "" ){
									 
								if(!(!tipoEvento && document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t_"+id3+"_t").checked == false)){
									document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t_"+id3+"_t").click();
								}
	
								if (tipoEvento == true){
									llenaCadena("jqg_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t_"+id3);
								}else {
									vaciaCadena("jqg_"+idUno+"_"+idDos+"_t_"+id+"_t_"+id2+"_t_"+id3);
								}
							}
						}
					}
		 ...
		 if(!(!tipoEvento && sub2.checked == false)){
			 ...
			 for(var j = 1; j<document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t").rows.length; j++){
			 	...
			 	if(document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t").rows[j].id == undefined || 
			 		...
			 		if(!(!tipoEvento && document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t").checked == false)){
			 			document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t").click();
			 		}
			 		
				 	for(var z = 1; z<document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t").rows.length; z++){
				 		
				 		var id3 = document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t").rows[z-1].id;
				 		if(document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t").rows[z].id == undefined || 
				 		   document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t").rows[z].id == "" ){
				 		   
				 		   if(!(!tipoEvento && document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t_"+id3+"_t").checked == false)){
				 			   document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t_"+id3+"_t").click();
				 		   }
				 			
				 		   if (tipoEvento == true){
				 			   llenaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t_"+id3);
						   }else {
							   vaciaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+id2+"_t_"+id3);
						   }
				 		}
				 	}
			 		   // TODO debo llenar la variable global 4 nivel
		 ...
		 if(!(!tipoEvento && sub3.checked == false)){
		 	...
		 	for(var z = 1; z<document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t").rows.length; z++){
		 		
		 		var id3 = document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t").rows[z-1].id;
		 		if(document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t").rows[z].id == undefined || 
		 		   document.getElementById(idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t").rows[z].id == "" ){
		 				 
		 		   if(!(!tipoEvento && document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+id3+"_t").checked == false)){
		 			   document.getElementById("cb_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+id3+"_t").click();
		 		   }
		 			
		 		   if(tipoEvento == true){
		 			   llenaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+id3);
		 		   } else {
		 			   vaciaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+id3);
		 		   }
		 		}
		 	}
	 ...
	 }else if(sub4 != undefined && arrayPalabras.length == 9){//nivel 4
		 if(!(!tipoEvento && sub4.checked == false)){
			 sub4.click();
			   
			if(tipoEvento == true){
				llenaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco);
			} else {
				vaciaCadena("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco);
			}
	 ...
	 if(arrayPalabras.length == 5){// nivel 2 valida 3 estados
		 ...
		 validaNivel4(idUno,idDos,idTres,idCuatro);		 
	 } else if (arrayPalabras.length == 11){//nivel 5
		 
		 var reg4Seleccion = 0; //valores verdaderos
		 if(arrCuartoNivel.length != undefined && arrCuartoNivel.length >0){
         
		 	for(var i =0; i<arrCuartoNivel.length; i++){
		 		if(arrCuartoNivel[i].idPadre != undefined && arrCuartoNivel[i].idPadre == "jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco){
 					if(arrCuartoNivel[i].selectAll != undefined && arrCuartoNivel[i].selectAll == "/"){
 						//todos son verdaderos
 						if(arrCuartoNivel[i].elimina != undefined && arrCuartoNivel[i].elimina.length >2){
 							reg4Seleccion = 2;
		 	...
		 	if(reg4Seleccion == 3){//si todos son falsos
				 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco).indeterminate = false;
				 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco).checked = false;
				 validaNivel4(idUno,idDos,idTres,idCuatro);
			 } else if(reg4Seleccion == 1){//si todos son verdaderos
				 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco).indeterminate = false;
				 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco).checked = true;
				 validaNivel4(idUno,idDos,idTres,idCuatro);
			 } else if(reg4Seleccion == 2){//si hay almenos un verdadero
				 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro+"_t_"+idCinco).indeterminate = true;
				 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres+"_t_"+idCuatro).indeterminate = true;
				 document.getElementById("jqg_"+idUno+"_"+idDos+"_t_"+idTres).indeterminate = true;
				 document.getElementById("jqg_"+idUno+"_"+idDos).indeterminate = true;		 
			 }
...
function buscarSelecciones5toNivel(idTableForm){
	...
	while(document.getElementById(strCadNivel1.replace("XX",cuentaNivel1))!=null && 
		...
		while(document.getElementById(strCadNivel2.replace("YY",cuentaNivel2))!=null && 
			...
			while(document.getElementById(strCadNivel3.replace("ZZ",cuentaNivel3))!=null && 
				...
				//rastreo por el cuarto nivel
				var cuentaNivel4=1;
				var strCadNivel4="jqg_"+idTableForm+"_"+cuentaNivel1+"_t_"+cuentaNivel2+"_t_"+cuentaNivel3+"_t_FF"; //cadena con el nombre del check
				
				while(document.getElementById(strCadNivel4.replace("FF",cuentaNivel4))!=null && 
					  document.getElementById(strCadNivel4.replace("FF",cuentaNivel4))!=undefined){
					
					if(document.getElementById(strCadNivel4.replace("FF",cuentaNivel4)).checked || 
					   document.getElementById(strCadNivel4.replace("FF",cuentaNivel4)).indeterminate==true){
						cadIdsSeleccion+="**4-"+document.getElementById(strCadNivel4.replace("FF",cuentaNivel4)).parentNode.parentNode.children[2].title;
					}				
				
					//buscamos elementos del 5to nivel
					for(var j=0; j<arrCuartoNivel.length; j++){
						if(arrCuartoNivel[j].idPadre==strCadNivel4.replace("FF",cuentaNivel4)){
							//si no tiene la seleccion de todos los elementos
							if(arrCuartoNivel[j].selectAll!='/'){
								cadIdsSeleccion+="**5-"+arrCuartoNivel[j].selecciones;
							}else{
								//en caso de que tenga la seleccion de todos los elementos
								cadIdsSeleccion+="**5-/"+arrCuartoNivel[j].elimina;
							}
							
							if(cadIdsSeleccion=="**5-" || cadIdsSeleccion=="**5-/"){//evitamos que la cadena se llene con basura en el 5to nivel
								cadIdsSeleccion="";
							}
						}
					}
...
/*************************** REGLAS PARA EL PAGINADO EN EL PRIMER NIVEL ****************************/
function selectAll1Nivel(element) {
	...
	var sub0 = document.getElementById("cb_"+idUno);
	 ...
	 if(arrayPalabras.length == 3){// nivel 1
		 
		 if(idDos != undefined){
			 if (tipoEvento == true) {
				llenaCadena("jqg_" + idUno + "_" + idDos);
			 } else {
				vaciaCadena("jqg_" + idUno + "_" + idDos);
			 }
		 }
	 }
}

/*************************** REGLAS PARA EL PAGINADO EN EL SEGUNDO NIVEL ****************************/
function selectAll2Nivel(element) {
	...
	} else if (arrayPalabras.length == 5) {// segundo nivel
		
		 if(arrCuartoNivel.length != undefined && arrCuartoNivel.length >0){
			 
			for(var i =0; i<arrCuartoNivel.length; i++){
				if(arrCuartoNivel[i].idPadre != undefined && arrCuartoNivel[i].idPadre == "jqg_"+idUno+"_"+idDos){
					if(arrCuartoNivel[i].selectAll != undefined && arrCuartoNivel[i].selectAll == "/"){
						//todos son verdaderos
						if(arrCuartoNivel[i].elimina != undefined && arrCuartoNivel[i].elimina.length >2){
							reg4Seleccion = 2;
...
function buscarSelecciones2doNivel(idTableForm){
	...
	while(document.getElementById(strCadNivel1.replace("XX",cuentaNivel1))!=null && 
		...
		for(var j=0; j<arrCuartoNivel.length; j++){
			if(arrCuartoNivel[j].idPadre==strCadNivel1.replace("XX",cuentaNivel1)){
				//si no tiene la seleccion de todos los elementos
				if(arrCuartoNivel[j].selectAll!='/'){
					cadIdsSeleccion+="**2-"+arrCuartoNivel[j].selecciones;
				}else{
					//en caso de que tenga la seleccion de todos los elementos
					cadIdsSeleccion+="**2-/"+arrCuartoNivel[j].elimina;
				}
				
				if(cadIdsSeleccion=="**2-" || cadIdsSeleccion=="**2-/"){//evitamos que la cadena se llene con basura en el 4to nivel
					cadIdsSeleccion="";
				}
